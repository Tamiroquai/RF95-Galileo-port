...
public:
    ...
    RH_RF95(uint8_t slaveSelectPin, uint8_t interruptPin = 2);
    ...
    virtual uint8_t    send(const uint8_t* data, uint8_t len);
    ...
    uint8_t        setFrequency(float centre);
...
/// TO header in the last received mesasge
    ...
    /// Low level interrupt service routine for device connected to interrupt 1
    static void         isr2(void* args);


    /// Array of instances connected to interrupts 0 and 1
    static RH_RF95*     _RF95ForInterrupt[];
